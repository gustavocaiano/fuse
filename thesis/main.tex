...
\usepackage{float} % Required for [H] table positioning

\usepackage[hidelinks]{hyperref} % no borders on links

% Allow embedding annex files (e.g., XLSX) inside the final PDF
% Load AFTER hyperref.
...
%\usepackage[style=authoryear-comp,backend=bibtex]{biblatex} % Bibtex backend with the authoryear-comp citation style (authoryear citations, bibliography ordered alphabetically)

% numeric citation style (e.g. [1], [1-3])
\usepackage[style=numeric,sorting=none,backend=bibtex]{biblatex} % Bibtex backend with the numeric-comp citation style (numeric citations, bibliography ordered by appearance)
...
\addbibresource{ch2/lrrq1.bib} % Referências específicas da LRRQ1
\addbibresource{ch2/lrrq2.bib} % Referências específicas da LRRQ2
\addbibresource{ch2/vl.bib} % Referências específicas da VL
...
\thesistitle{Sistema de Integração Segura e Análise Automática de CCTV com Câmaras em Redes Não Controladas} % Your thesis title, this is used in the title, print it elsewhere with \ttitle

%\thesissubtitle{{[}Sistema de Integração e Análise Centralizada de CCTV{]}} % Your thesis title, this is used in the title, print it elsewhere with \tsubtitle

\author{Gustavo \textsc{Caiano}} % Your name, this is used in the title page, print it elsewhere with \authorname
...
\advisor{Paulo Baltarejo Sousa} % Your advisors's (academic mentor) name, this is used in the title page, print it elsewhere with \advname
...
\supervisor{Francisco Loureiro} % Your supervisor's (company, hosting institution mentor) name, this is used in the title page, print it elsewhere with \supname (comment, if no supervisor, or same as advisor)
...
\keywords{Videovigilância, Integração CCTV, Automatização, Deteção de Objetos, Software Seguro, IoT Communication} % Please define up to 6 keywords that better describe your work, print it elsewhere with \keywordnames
...
\input{ch1/chapter1}
\input{ch2/chapter2}
%\input{ch3/chapter3} % Conteúdo movido para secção 1.6 do Capítulo 1
%\input{ch4/chapter4} % Conteúdo movido para secção Planeamento do Capítulo 1 e apêndices C e D
\input{ch5/chapter5}
